...
import { ArrowLeft, Edit, FileText, Home, Plus } from 'lucide-react';
...
export default function SchoolDetailPage() {
  ...
  const fetchSchoolData = async () => {
    try {
      const res = await fetch(`/api/school-admin/schools/${params.id}`);
      if (res.ok) {
        const data = await res.json();
        setSchool(data.school);
        setSchoolStats(data.stats)
      } else {
        router.push('/school-admin/schools');
      }
    } catch (error) {
  ...
  const fetchAdmins = async () => {
    try {
      const res = await fetch(`/api/school-admin/schools/${params.id}/admins`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`/api/school-admin/schools/${params.id}/history`);
      if (res.ok) {
        const data = await res.json();
        setLogs(data.logs);
      }
  ...
  const fetchPracticeData = async () => {
    try {
      const res = await fetch(`/api/school-admin/schools/${params.id}/practice`);
      if (res.ok) {
        const data = await res.json();
  ...
  return (
    <div className={`flex min-h-screen bg-gray-50`}>
      ...
      <main className="flex-1 px-8 py-8">
        ...
        <div className={`mb-8 `}>
          <button
            onClick={() => router.push('/school-admin/schools')}
...
function OverviewTab({ id, admins, isRTL, dailyActivity, hourlyActivity, }: {
  id: string | number;
  admins: Admin[];
  isRTL: boolean;
  hourlyActivity: HourlyActivity[];
