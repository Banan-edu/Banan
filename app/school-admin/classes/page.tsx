...
export default function InstructorClassesPage() {
    ...
    try {
      ...
      const classesWithActivity = await Promise.all(
        data.classes.map(async (classItem: any) => {
          try {
            const activityRes = await fetch(`/api/school-admin/classes/${classItem.id}/activity`);
            if (activityRes.ok) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`/api/school-admin/classes/${classToDelete.id}`, {
        method: 'DELETE',
      });
    ...
    <div className={`flex min-h-screen bg-gray-50`}>
      ...
      <main className="flex-1 px-8 py-8">
        <div className={`flex justify-between items-center mb-8`}>
          ...
          <button
            onClick={() => router.push('/school-admin/classes/add')}
