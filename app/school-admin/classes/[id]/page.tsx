...
export default function ClassDetailsPage() {
  ...
  const fetchClassData = async () => {
    try {
      const res = await fetch(`/api/school-admin/classes/${classId}`);
      if (!res.ok) {
      ...
      <main className="flex-1 px-8 py-8">
        <div className="mb-6">
          ...
          <div className='flex  justify-between items-center'>
            ...
            <button
              onClick={() => router.push(`/school-admin/classes/${classData.id}/edit`)}
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'live' && (
            <LiveActivityTab 
              api={'school-admin'} classId={classId} isRTL />
          )}
        </div>
      </main>
