...
export default function EditClassPage() {
  ...
  const fetchData = async () => {
    try {
      // Fetch class data
      const classRes = await fetch(`/api/school-admin/classes/${params.id}/edit`);
      if (!classRes.ok) {
      ...
      // Fetch instructors
      const instructorsRes = await fetch('/api/school-admin/instructors?role=instructor');
      ...
      // Fetch courses
      const coursesRes = await fetch('/api/school-admin/courses');
      if (coursesRes.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/school-admin/classes/${params.id}/edit`, {
        method: 'PUT',
      ...
      if (res.ok) {
        router.push(`/school-admin/classes/${params.id}`);
      } else {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl shadow-md p-6">
            ...
            <div className="flex gap-4 pt-6">
              ...
              <button
                type="button"
                onClick={() => router.push(`/school-admin/classes/${params.id}`)}
