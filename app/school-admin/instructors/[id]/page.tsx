...
export default function InstructorDetailPage() {
    ...
    const fetchInstructorData = async () => {
        try {
            const [instructorRes, classesRes, historyRes, allClassesRes, schoolsRes] = await Promise.all([
                fetch(`/api/school-admin/instructors/${instructorId}`),
                fetch(`/api/school-admin/instructors/${instructorId}/classes`),
                fetch(`/api/school-admin/instructors/${instructorId}/history`),
                fetch('/api/school-admin/classes'),
                fetch('/api/school-admin/schools')
            ]);
    ...
    const handleAddClasses = async () => {
        try {
            const res = await fetch(`/api/school-admin/instructors/${instructorId}/classes`, {
                method: 'POST',
    ...
    const handleDelete = async (classId: number) => {
        ...
        try {
            const res = await fetch(`/api/school-admin/instructors/${instructorId}/classes/${classId}`, {
                method: 'DELETE',
            });
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`/api/school-admin/instructors/${instructorId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            <Sidebar links={adminLinks} userRole="school-admin" />

            <main className="flex-1 px-8 py-8">
                <div className="mb-8">
                    <button
                        onClick={() => router.push('/school-admin/instructors')}
                                        ...
                                        <div className="space-y-3">
                                            <div className={`flex items-center gap-2  `}>
                                                ...
                                                <span className={`text-sm text-gray-700 ${isRTL ? 'font-arabic' : ''}`}>
                                                    {isRTL ? 'الوصول إلى جميع الطلاب من جميع مدارسي' : 'Access all students from all my schools'}
                                                </span>
                                            </div>
                                            <div className={`flex items-center gap-2  `}>
                                                <Checkbox
