...
export default function SchoolAdminInstructorsPage() {
  ...
  const fetchData = async () => {
    try {
      const [instructorsRes, schoolsRes] = await Promise.all([
        fetch('/api/school-admin/instructors'),
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch('/api/school-admin/instructors', {
        method: 'POST',
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/school-admin/instructors/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        fetchData();
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {instructors.map((instructor) => (
                <tr key={instructor.id} className="hover:bg-gray-50 cursor-pointer">
                  <td className="px-6 py-4 whitespace-nowrap" onClick={() => router.push(`/school-admin/instructors/${instructor.id}`)}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 ${isRTL ? 'font-arabic' : ''}`}>
                      {isRTL ? (instructor.role === 'instructor' ? 'معلم' : instructor.role === 'school_admin' ? 'مدير مدرسة' : 'مدير فواتير') : instructor.role.replace('_', ' ')}
                    </span>
                  </td>
