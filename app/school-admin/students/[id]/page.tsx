            ...
            <main className="flex-1 px-8 py-8">
                <div className="mb-6">
                    ...
                    <div className='flex  justify-between items-center'>
                        ...
                        <button
                            onClick={() => router.push(`/school-admin/students/${student.user.id}/edit`)}
                ...
                <div className="bg-white rounded-lg shadow-sm mb-6">
                            ...
                            {tabs.map((tab) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`py-4 px-3 border-b-2 font-medium text-sm whitespace-nowrap flex items-center gap-2 ${activeTab === tab.id
                    ...
                    <div className="p-6">
                        {activeTab === 'overview' && <OverviewTab student={student} studentId={studentId} isRTL={isRTL} />}
...
function OverviewTab({ student, studentId, isRTL }: { student: any; studentId: string; isRTL: boolean }) {
    const router = useRouter();

    const handleLoginAsStudent = async () => {
        try {
            const res = await fetch(`/api/school-admin/students/${student.user.id}/login-as`, {
                method: 'POST',
            });
            if (res.ok) {
                router.push('/student');
    ...
    const fetchPerformance = async () => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/performance`);
            if (res.ok) {
                const data = await res.json();
                setPerformance(data);
            }
    ...
    const fetchHistory = async () => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/history`);
            if (res.ok) {
                const data = await res.json();
    ...
    const fetchBadges = async () => {
        try {
            const [earnedRes, availableRes] = await Promise.all([
                fetch(`/api/school-admin/students/${studentId}/badges`),
                fetch(`/api/school-admin/badges`),
            ]);
            if (earnedRes.ok) {
    ...
    const assignBadge = async (badgeId: number) => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/badges`, {
                method: 'POST',
    ...
    const fetchPracticeData = async () => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/practice`);
            if (res.ok) {
                const data = await res.json();
    ...
    if (performanceLoading || historyLoading || badgesLoading || practiceLoading) return <div>Loading...</div>;
    ...
    return (
        <div className="space-y-12">
            ...
            {/* History Section */}
            <div className="space-y-6">
                <h3 className="text-2xl font-bold text-gray-900">{isRTL ? 'السجل' : 'History'}</h3>
            ...
            {/* Badges Section */}
            <div className="space-y-6">
                <h3 className="text-2xl font-bold text-gray-900">{isRTL ? 'الشارات' : 'Badges'}</h3>
                <div>
                    <h4 className="text-lg font-semibold mb-4">{isRTL ? 'الشارات المكتسبة' : 'Earned Badges'}</h4>
                ...
                <button
                    ...
                    className="w-20 h-20 flex items-center justify-center rounded-full bg-blue-500 text-white shadow-md hover:bg-blue-600 transition-all"
                    title={isRTL ? "عرض الشارات" : "Show badges"}
                >
                    <Plus className="w-6 h-6" />
                </button>

                {showBadges && (
                    <div className="mt-6 p-4 border border-gray-200 rounded-xl bg-white shadow-sm animate-fadeIn">
                        <div className="flex items-center justify-between mb-4">
                            <h4 className="text-lg font-semibold">
                                {isRTL ? "منح شارة" : "Assign Badge"}
                            </h4>
                            <button
            ...
            {/* Practice Section */}
            <div className="space-y-6">
                <h3 className="text-2xl font-bold text-gray-900">{isRTL ? 'التمرين' : 'Practice'}</h3>
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                    <h4 className="text-lg font-semibold mb-4">{isRTL ? 'تحليلات التمرين' : 'Practice Analytics'}</h4>
...
function ClassesTab({ studentId, isRTL, fetchStudentData }: { studentId: string; isRTL: boolean; fetchStudentData: () => void }) {
    ...
    const fetchClasses = async () => {
        try {
            const [enrolledRes, availableRes] = await Promise.all([
                fetch(`/api/school-admin/students/${studentId}/classes`),
    ...
    const disenrollClass = async (classId: number) => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/classes/${classId}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                fetchClasses();
    ...
    const enrollClass = async (classId: number) => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/classes`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function StudentProgressTab({ studentId, isRTL }: { studentId: string; isRTL: boolean }) {
    ...
    const fetchProgress = async () => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/progress`);
            if (res.ok) {
                const data = await res.json();
    ...
    const clearProgress = async (courseId: number) => {
        ...
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/progress/${courseId}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                fetchProgress();
...
function ActivityTab({ studentId, isRTL }: { studentId: string; isRTL: boolean }) {
    ...
    const fetchActivity = async () => {
        try {
            const res = await fetch(`/api/school-admin/students/${studentId}/activity`);
            if (res.ok) {
                const data = await res.json();
