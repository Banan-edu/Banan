...
export async function GET() {
  ...
  try {
    // Get tests created by this instructor or where they are assigned
    const instructorTests = await db
      .select({ testId: testInstructors.testId })
      .from(testInstructors)
      .where(eq(testInstructors.userId, session.userId));

    const testIds = instructorTests.map(t => t.testId);
    ...
    const testsData = await db
      ...
      .where(
        or(
          eq(tests.createdBy, session.userId),
          testIds.length > 0 ? inArray(tests.id, testIds) : undefined
        )
      );
