...
export default function InstructorCourseDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    if (deleteModal.type === 'section') {
      const res = await fetch(`/api/instructor/courses/${courseId}/sections/${deleteModal.item.id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        setDeleteModal(null);
      ...
      const res = await fetch(`/api/instructor/courses/${courseId}/lessons/${deleteModal.item.id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        setDeleteModal(null);
