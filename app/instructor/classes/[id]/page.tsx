...
type Tab = 'overview' | 'settings' | 'performance' | 'history' | 'activity' | 'students' | 'instructors' | 'courses' | 'scoreboard' | 'live';
...
export default function ClassDetailsPage() {
  ...
  const tabs = [
    { id: 'overview' as Tab, label: isRTL ? 'نظرة عامة' : 'Overview', icon: Users },
    { id: 'settings' as Tab, label: isRTL ? 'الإعدادات' : 'Settings', icon: Settings },
    { id: 'performance' as Tab, label: isRTL ? 'الأداء' : 'Performance', icon: TrendingUp },
    { id: 'history' as Tab, label: isRTL ? 'السجل' : 'History', icon: FileText },
    { id: 'activity' as Tab, label: isRTL ? 'النشاط' : 'Activity', icon: Activity },
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="mt-6">
                <label className="flex items-center gap-3">
                  <span className="font-medium">{isRTL ? 'رمز الصف' : 'Class Code'}:</span>
                  <input type="checkbox" className="toggle" />
                  <span className="text-sm text-gray-600">{isRTL ? 'مفعّل/معطّل' : 'Enabled/Disabled'}</span>
                </label>
              </div>
            </div>
          )}
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              <h2 className="text-xl font-semibold mb-4">{isRTL ? 'إعدادات الصف' : 'Class Settings'}</h2>
          ...
          {activeTab === 'performance' && (
            <div className="space-y-6">
              <h2 className="text-xl font-semibold mb-4">{isRTL ? 'أداء الكتابة' : 'Typing Performance'}</h2>
          ...
          {activeTab === 'history' && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold mb-4">{isRTL ? 'سجل الأحداث' : 'Record History'}</h2>
          ...
          {activeTab === 'activity' && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold mb-4">{isRTL ? 'نشاط الصف' : 'Class Activity'}</h2>
          ...
          {activeTab === 'students' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">{isRTL ? 'الطلاب' : 'Students'}</h2>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left">{isRTL ? 'الصعوبة' : 'Difficulty'}</th>
                      <th className="px-4 py-3 text-left">{isRTL ? 'الصف' : 'Grade'}</th>
                      <th className="px-4 py-3 text-left">{isRTL ? 'آخر نشاط' : 'Last Activity'}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-t">
                      <td className="px-4 py-3 text-gray-500" colSpan={4}>No students enrolled</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left">{isRTL ? 'الاسم' : 'Name'}</th>
                      <th className="px-4 py-3 text-left">{isRTL ? 'البريد' : 'Email'}</th>
                    ...
                    {classData.instructors.map((instructor: any) => (
                      <tr key={instructor.userId} className="border-t">
                        ...
                        <td className="px-4 py-3">
                          <button className="text-red-600 hover:text-red-700">Unassign</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
