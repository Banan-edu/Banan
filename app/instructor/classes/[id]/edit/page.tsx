...
export default function EditClassPage() {
  ...
  const fetchData = async () => {
    try {
      // Fetch class data
      const classRes = await fetch(`/api/instructor/classes/${params.id}/edit`);
      if (!classRes.ok) {
      ...
      // Fetch instructors
      const instructorsRes = await fetch('/api/instructor/instructors');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/instructor/classes/${params.id}/edit`, {
        method: 'PUT',
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl shadow-md p-6">
            ...
            <div className="flex gap-4 pt-6">
              ...
              <button
                type="button"
                onClick={() => router.push(`/instructor/classes/${params.id}`)}
