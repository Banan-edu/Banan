            ...
            <main className="flex-1 px-8 py-8">
                <div className="mb-6">
                    ...
                    <div className='flex  justify-between items-center'>
                        ...
                        <button
                            onClick={() => router.push(`/admin/students/${student.user.id}/edit`)}
...
function OverviewTab({ student, studentId, isRTL }: { student: any; studentId: string; isRTL: boolean }) {
    ...
    const handleLoginAsStudent = async () => {
        try {
            const res = await fetch(`/api/admin/students/${student.user.id}/login-as`, {
                method: 'POST',
            });
            if (res.ok) {
                router.push('/student');
    ...
    const fetchPerformance = async () => {
        try {
            const res = await fetch(`/api/admin/students/${studentId}/performance`);
            if (res.ok) {
                const data = await res.json();
                setPerformance(data);
            }
    ...
    const fetchHistory = async () => {
        try {
            const res = await fetch(`/api/admin/students/${studentId}/history`);
            if (res.ok) {
                const data = await res.json();
    ...
    const fetchBadges = async () => {
        try {
            const [earnedRes, availableRes] = await Promise.all([
                fetch(`/api/admin/students/${studentId}/badges`),
    ...
    const fetchPracticeData = async () => {
        try {
            const res = await fetch(`/api/admin/students/${studentId}/practice`);
            if (res.ok) {
                const data = await res.json();
...
function ClassesTab({ studentId, isRTL, fetchStudentData }: { studentId: string; isRTL: boolean; fetchStudentData: () => void }) {
    ...
    const fetchClasses = async () => {
        try {
            const [enrolledRes, availableRes] = await Promise.all([
                fetch(`/api/admin/students/${studentId}/classes`),
    ...
    const disenrollClass = async (classId: number) => {
        try {
            const res = await fetch(`/api/admin/students/${studentId}/classes/${classId}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                fetchClasses();
    ...
    const enrollClass = async (classId: number) => {
        try {
            const res = await fetch(`/api/admin/students/${studentId}/classes`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function StudentProgressTab({ studentId, isRTL }: { studentId: string; isRTL: boolean }) {
    ...
    const clearProgress = async (courseId: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/students/${studentId}/progress/${courseId}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                fetchProgress();
