...
import { adminLinks, instructorLinks, Sidebar } from '@/components/Sidebar';
...
export default function InstructorCourseDetailPage() {
  ...
  const handleSectionSubmit = async (name: string) => {
    if (sectionModalMode === 'add') {
      ...
      const res = await fetch(`/api/admin/courses/${courseId}/sections/${selectedSection.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name }),
      });
  ...
  const handleLessonSubmit = async (data: any) => {
    if (lessonModalMode === 'add') {
      ...
      const res = await fetch(`/api/admin/courses/${courseId}/lessons/${selectedLesson.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    ...
    if (deleteModal.type === 'section') {
      const res = await fetch(`/api/admin/courses/${courseId}/sections/${deleteModal.item.id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        setDeleteModal(null);
      ...
      const res = await fetch(`/api/admin/courses/${courseId}/lessons/${deleteModal.item.id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        setDeleteModal(null);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeType === 'section' && overType === 'section') {
      ...
      await fetch(`/api/admin/courses/${courseId}/sections/reorder`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
