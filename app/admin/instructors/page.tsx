  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/users', {
        ...
        body: JSON.stringify({ ...formData, role: 'instructor', schoolId: parseInt(formData.schoolId) }),
      });
      if (res.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center space-x-4">
              ...
              <span className="text-gray-600">Admin - Instructors</span>
            </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {instructors.map((instructor) => (
                <tr key={instructor.id}>
                  <td className="px-6 py-4 whitespace-nowrap"><div className="flex items-center"><UserCheck className="w-5 h-5 text-blue-600 mr-2" />{instructor.name}</div></td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600">{instructor.email}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600">{instructor.schoolId ? schools.find(s => s.id === instructor.schoolId)?.name : 'N/A'}</td>
                </tr>
              ))}
            </tbody>
          </table>
